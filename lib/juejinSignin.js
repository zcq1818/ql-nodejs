...
export async function juejinSignin(cookie, { timeout = 50 } = {}) {
  ...
  try {
    ...
    page.on('response', async (resp) => {
      const u = resp.url();
      // 只匹配真正的签到 POST（check_in?aid=），排除页面加载时的状态/规则查询
      if (u.includes('/growth_api/v1/check_in?') && resp.request().method() === 'POST') {
        try {
          const j = await resp.json();
          if (j && j.err_no === 0) {
            checkInData = j.data ?? null;
            log(`[check_in] 签到成功 err_no=0`);
          } else {
            checkInErr = j?.err_msg || `err_no=${j?.err_no}`;
            log(`[check_in] 签到失败 err_no=${j?.err_no} msg=${j?.err_msg || ''}`);
          }
        } catch {}
      }
    ...
    // 注入 cookie（context.addCookies 后，后续所有导航自动携带）
    ...
    for (const ck of cookies) {
      try {
        ...
        failCount++;
        skippedNames.push(ck.name);
      }
    }
    log(`[cookie] 注入 ${okCount} 条成功${failCount ? `，${failCount} 条跳过（格式问题）` : ''}`);
    if (skippedNames.length) {
      log(`[cookie] 跳过的 cookie: ${skippedNames.join(', ')}`);
    }

    // 直接打开签到页（cookie 已在 context 上，无需先开首页再 reload）
    ...
    if (clicked) {
      ...
      if (checkInErr) {
        log('签到失败：' + checkInErr);
        if (checkInErr.includes('must login') || checkInErr.includes('登录')) {
          log('[hint] 登录态未生效，可能是关键 cookie（如 sessionid）被跳过或 cookie 已过期。请重新从浏览器复制最新 cookie 更新 JUEJIN_COOKIE 环境变量。');
        }
      } else if (checkInData && checkInData.incr_point != null) {
        log(`签到成功 +${checkInData.incr_point} 矿石，当前 ${checkInData.sum_point} 矿石 🎉`);
      } else {
        log('已点击签到，但未捕获到有效签到响应（可能今日已签到或触发风控）');
      }
    } else {
      // 未找到签到按钮，检查是否已签到（登录态有效但今天已签）
      if (checkInErr && checkInErr.includes('must login')) {
        log('登录态未生效（must login），cookie 可能已过期或关键 cookie 被跳过');
      } else {
